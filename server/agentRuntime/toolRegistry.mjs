...
function makeSignal(context, timeoutMs) {
  ...
  if (typeof AbortSignal.any === "function") {
    ...
    for (const source of [context.signal, timeoutController.signal]) {
...
export function createToolRegistry({
  ...
  return Object.freeze({
    ...
    async execute({ call, request, runtimeMode, externalStepIndex, callsInStep, context }) {
      ...
      const { signal, timeoutController, timeoutId } = makeSignal(context, record.timeoutMs);
      const analyticsFetchWithSignal = (url, init = {}) => analyticsFetch(url, { ...init, signal });
      try {
        const result = await executeLegacyTool(toLegacyToolCall(call, parsedArgs), { analyticsFetch: analyticsFetchWithSignal, analyticsApiBase, runDuplicateBridge, ensureDuplicateWarmup });
