...
export const resumeAgentRun = async ({ fetchImpl = fetch, runId, interactionId, threadVersion, value }: any) => parseResponse(await fetchImpl(`/api/agent/runs/${encodeURIComponent(runId)}/resume`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ schemaVersion: "1.0", interactionId, threadVersion, value }) }));
