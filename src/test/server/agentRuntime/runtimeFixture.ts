...
export async function createTestRuntime() {
  ...
  const runtime = createAgentRuntime({ db: fixture.runtimeDb.db, contracts, threadStore, eventStore: events, auditStore: audit, policy: createRuntimePolicy(), now: clock.now, nowMs: clock.nowMs, randomUUID: () => `id-${++id}`, executeClaimedRun: async () => undefined });
