  ...
  return {
    ...
    properties: {
      ...
      input: {
        ...
        required: ["plan_ref"],
        additionalProperties: false,
        properties: { plan_ref: { type: "string", minLength: 1 } },
      },
    },
  };
}
...
export function expandPrimitiveToolCall(toolCall, { governedQueryPlan } = {}) {
  ...
  if (primitive === "analyze" && operation === "semantic_metrics" && adapterInput?.plan_ref) {
    ...
    const steps = (Array.isArray(governedQueryPlan?.steps) ? governedQueryPlan.steps : [])
      .filter((step) => step?.toolName === "query_semantic_metrics" && step?.operation === "semantic_metric_query");
    if (steps.length !== 1 || !steps[0]?.canonicalArgs) throw new Error("PRIMITIVE_PLAN_STEP_INVALID");
    adapterInput = steps[0].canonicalArgs;
  }
  if (primitive === "trace") {
    if (!adapterInput?.plan_ref) throw new Error("PRIMITIVE_INPUT_INVALID:trace:direct");
    if (String(governedQueryPlan?.planId || "") !== String(adapterInput.plan_ref)) throw new Error("PRIMITIVE_PLAN_REF_INVALID");
    const steps = (Array.isArray(governedQueryPlan?.steps) ? governedQueryPlan.steps : [])
      .filter((step) => step?.toolName === "query_traceability" && step?.operation === "traceability_query");
    if (steps.length !== 1 || !steps[0]?.canonicalArgs) throw new Error("PRIMITIVE_PLAN_STEP_INVALID");
    adapterInput = steps[0].canonicalArgs;
  }
  if (primitive === "records" && operation === "semantic") {
    if (adapterInput?.plan_ref) {
      if (String(governedQueryPlan?.planId || "") !== String(adapterInput.plan_ref)) throw new Error("PRIMITIVE_PLAN_REF_INVALID");
      const steps = (Array.isArray(governedQueryPlan?.steps) ? governedQueryPlan.steps : [])
        .filter((step) => step?.toolName === "query_semantic_records" && step?.operation === "semantic_record_query");
      if (steps.length !== 1 || !steps[0]?.canonicalArgs) throw new Error("PRIMITIVE_PLAN_STEP_INVALID");
      adapterInput = steps[0].canonicalArgs;
