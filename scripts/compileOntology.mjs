...
const documentDefinitions = Object.freeze([
  ...
  { file: "business_rules.json", schema: "business_rules.schema.json", key: "businessRules" },
  ...
  { file: "actions.json", schema: "actions.schema.json", key: "actions" },
]);
...
export function compileOntology({
  ...
  const bundle = canonicalize({
    ...
    businessRules: sortById(documents.businessRules),
    policies: sortById(documents.policies),
    constraints: sortById(documents.constraints),
    actions: sortById(documents.actions),
  });
